...
class zkNet(object):
        ...
        if type(self.meta['learning_rate']) is dict:
            ...
            if learningRateMeta["type"] != 'pol':
                self.learning_rate = self.learningRateOpt[learningRateMeta["type"]](
                    learning_rate = learningRateMeta['learning_rate'],
                    global_step = self.global_step,
                    decay_steps = learningRateMeta['decay_steps'],
                    decay_rate = learningRateMeta['decay_rate'],
                    staircase = True
                )
            else:
                self.learning_rate = self.learningRateOpt[learningRateMeta["type"]](
                    learning_rate = learningRateMeta['learning_rate'] if 'learning_rate' in learningRateMeta else 0.1,
                    global_step = self.global_step,
                    decay_steps = learningRateMeta['decay_steps'] if 'decay_steps' in learningRateMeta else 10,
                    decay_rate = learningRateMeta['decay_rate'] if 'decay_rate' in learningRateMeta else 0.01,
                    end_learning_rate = learningRateMeta['end_learning_rate'] if 'end_learning_rate' in learningRateMeta else 0.0001,
                    power = 0,
                    cycle = True
                )
        else:
    ...
    def build_forward(self):
        ...
        self.cost = self.lossFunc(self.logits, self.label)
        self.acc = self.accFunc(self.logits, self.label)

    def LossFunction(self, logits, label):
        ...
        return ce + L2
    def AccFunction(self, logits, label):
            ...
            if self.meta['isSaveNpzFile'] == 2: # fune tune
                ...
                if self.meta['trainType'] == 0:
                    for epoch in range(int(self.meta['epoch'])):
                        ...
                        if print_test_acc > max_test_accuracy:
                            max_test_accuracy = print_test_acc
                        ...
                        print("epoch %3d" % epoch, ", train_loss %.3f" % train_loss_value,
                              ...
                              ", max test accuracy %.2f %%" % (max_test_accuracy * 100))
